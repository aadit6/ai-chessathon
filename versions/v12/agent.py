...
evaluation, killers, countermoves and history with penalties for quiet moves that failed,
null-move, reverse futility, futility and late move pruning,
...
# Between moves, a table this full drops what the last few searches did not write. A few searches
# write well under a quarter of the table, so it never has to be wiped mid-game.
TT_PRUNE_AT = TT_MAX_ENTRIES * 3 // 4
TT_KEEP_SEARCHES = 4
...
# depth, bound, score, best move, and the search that wrote it
TTEntry = tuple[int, int, int, chess.Move | None, int]
...
class Searcher:
    def __init__(self) -> None:
        ...
        # The quiet reply that last refuted each opponent move, by side to move, from and to.
        self.countermoves: list[chess.Move | None] = [None] * (2 * 64 * 64)
    ...
    def search(
        ...
        if entry is not None:
            entry_depth, flag, entry_score, table_move, _ = entry
        ...
        moves = self.staged(board, table_move, ply)
        ...
        try:
            for index, move in enumerate(moves):
                ...
                if score > alpha:
                    ...
                    if alpha >= beta:
                        if quiet:
                            self.remember_cutoff(board, move, depth, ply, tried)
                        break
                if quiet:
                    tried.append(move)
        finally:
        ...
        # The first move is never pruned, so no best move means there were no legal moves.
        ...
        # unless that one is left over from an earlier move.
        existing = self.table.get(key)
        if (
            existing is None
            or depth >= existing[0]
            or flag == EXACT
            or existing[4] != self.generation
        ):
            self.table[key] = (depth, flag, _to_table(best, ply), best_move, self.generation)
        return best

    def age_table(self) -> None:
        """Drop entries no recent search wrote, instead of wiping what the last moves learned."""
        oldest = self.generation - TT_KEEP_SEARCHES
        self.table = {key: entry for key, entry in self.table.items() if entry[4] > oldest}
        # Only if recent searches alone fill it does it fall back to starting empty.
        if len(self.table) >= TT_PRUNE_AT:
            self.table.clear()
    ...
    def staged(
        ...
        """The hash move first, and the rest only if it fails to cut the node off.

        Generating, scoring and sorting every legal move is the dearest step of a node, and the
        hash move alone settles most cut nodes, so the list is built only once it is needed.
        """
        if table_move is None or not board.is_legal(table_move):
            yield from self.order(board, None, ply)
            return
        yield table_move
        for move in self.order(board, None, ply):
            if move != table_move:
                yield move
    ...
    def order(
        ...
        if board.move_stack:
            ...
            if previous:  # a null move has no reply worth remembering
                counter = self.countermoves[base + previous.from_square * 64 + previous.to_square]
        ...
        def priority(move: chess.Move) -> int:
            ...
            if move == counter:
                return 150_000
            return max(-99_000, min(history[base + move.from_square * 64 + move.to_square], 99_000))
    ...
    def remember_cutoff(
        self, board: chess.Board, move: chess.Move, depth: int, ply: int, tried: list[chess.Move]
    ) -> None:
        killers = self.killers[ply]
        if move != killers[0]:
            killers[1] = killers[0]
        ...
        bonus = depth * depth
        history = self.history
        history[base + move.from_square * 64 + move.to_square] += bonus
        # Quiet moves searched before the one that cut failed here, so they rank lower next time.
        for failed in tried:
            history[base + failed.from_square * 64 + failed.to_square] -= bonus
        if board.move_stack:
            previous = board.move_stack[-1]
            if previous:
                self.countermoves[base + previous.from_square * 64 + previous.to_square] = move
